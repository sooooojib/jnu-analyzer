...
export const Alert: React.FC<AlertProps> = ({
  ...
  const configs = {
    info: {
      icon: Info,
      wrapper: "bg-sky-950/40 border-sky-800/60 text-sky-200",
  ...
  return (
    <div className={twMerge(clsx("relative flex items-start gap-3 p-4 rounded-xl border", current.wrapper, className))}>
      <IconComponent className={clsx("w-5 h-5 flex-shrink-0 mt-0.5", current.iconColor)} />
      <div className="flex-1 text-sm">
        {title && <div className="font-semibold mb-0.5 text-slate-100">{title}</div>}
